...
export default function City({ city, isCurrentCity, type, onClick }) {
  ...
  return (
    ...
    <div
      className={`noScrollbar flex-1 grid cursor-pointer grid-cols-[80px_1fr_auto]  items-center overflow-auto rounded-xl border-primary bg-background-secondary p-3 transition-colors duration-300 hover:border  hover:bg-transparent ${
        type === 2 ? 'min-w-[150px] place-items-center  grid-cols-none gap-3 px-5' : type === 3 ? 'py-1' : 'gap-5'
      } 
  ...
  {/* <button className='border grid place-content-center border-text-secondary hover:bg-primary hover:text-white hover:border-none text-text-secondary rounded-full w-6  font-bold h-6'>+</button> */}
